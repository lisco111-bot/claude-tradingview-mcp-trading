...
// Prevent opening files in Railway environment
if (process.env.RAILWAY_APP_ID || process.env.RAILWAY_ENVIRONMENT) {
  // Override execSync to prevent opening files
  const originalExecSync = execSync;
  execSync = (command, options) => {
    if (command.includes('open') && command.includes('.env')) {
      console.log("⚠️  Cannot open .env file in Railway environment");
      return Buffer.from('');
    }
...
function safeEnvCheck() {
  // Safe environment check without opening files
  const isRailway = process.env.RAILWAY_APP_ID || process.env.RAILWAY_ENVIRONMENT;

  if (isRailway) {
    console.log("Running in Railway environment - using environment variables");
    return true;
  }
...
const useInMemoryStorage = !!process.env.RAILWAY_APP_ID || process.env.NODE_ENV === "production" || process.env.RAILWAY_ENVIRONMENT;
...
function loadLog() {
  // Skip file operations for Railway deployment
...
function initCsv() {
  // Skip file creation for Railway deployment
  if (useInMemoryStorage) {
    console.log(
      `📄 Using in-memory storage for trades (Railway environment)`,
    );
    return;
  }
...
async function writeTradeCsv(logEntry) {
  ...
  // Skip file operations for Railway
  if (useInMemoryStorage) {
